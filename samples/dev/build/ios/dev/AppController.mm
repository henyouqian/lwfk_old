...
- (void) applicationDidFinishLaunching:(UIApplication*)application
{
	...
	pPVRShell->PVRShellSet(prefWidth, rect.size.width*scale);
	pPVRShell->PVRShellSet(prefHeight, rect.size.height*scale);
	...
	for(;;)
	{
		...
		_glView = [[EAGLView alloc] initWithFrame:rect pixelFormat:strColourFormat depthFormat:iDepthFormat stencilFormat:iStencilFormat preserveBackbuffer:NO scale:scale msaaMaxSamples:iMSAA];	// i.e. 1,2,4 for MSAA
