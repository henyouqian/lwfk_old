...
static int32_t handle_input(struct android_app* app, AInputEvent* event)
{
	...
	if(init)
	{
		...
		for (int i = 0; i < pointerCount; ++i) {
			...
			float prevX = AMotionEvent_getHistoricalX(event, i, 0);
			float prevY = AMotionEvent_getHistoricalY(event, i, 0);
			...
			switch(AInputEvent_getType(event))
			{
				...
				{
					switch(AMotionEvent_getActionMasked(event))
					{
