...
void PVRShellInit::OsDisplayDebugString(char const * const str)
{
	...
	__android_log_print(ANDROID_LOG_INFO, pszAppName ? pszAppName : "PVRShell", str);
}
...
static int32_t handle_input(struct android_app* app, AInputEvent* event)
{
	...
	if (type == AINPUT_EVENT_TYPE_MOTION) {
		int rawAction = AMotionEvent_getAction(event);
		int action = rawAction & AMOTION_EVENT_ACTION_MASK;
		int pointIndex = (rawAction & AMOTION_EVENT_ACTION_POINTER_INDEX_MASK)>>8;
		...
		if (action == AMOTION_EVENT_ACTION_DOWN || action == AMOTION_EVENT_ACTION_POINTER_DOWN) {
			...
			for (int i = 0; i < pointerCount; ++i) {
				int key = AMotionEvent_getPointerId(event, i);
				float x = AMotionEvent_getX(event, i);
				float y = AMotionEvent_getY(event, i);
				float prevX = AMotionEvent_getHistoricalX(event, i, 1);
				float prevY = AMotionEvent_getHistoricalY(event, i, 1);
	...
	// std::map<int, Touch>& touchMap = lw::getTouchMap();
	// std::map<int, Touch>::iterator it = touchMap.begin();
	// for (; it != touchMap.end(); ) {
	// 	bool found = false;
	// 	for (int i = 0; i < pointerCount; ++i) {
	// 		int key = AMotionEvent_getPointerId(event, i);
	...
	// lwinfo("xxx:%d", AMotionEvent_getPointerCount(event));
